 ...
 *
 * Two modes:
 * 1. JSON body with { filename, contentType, brandSlug, assetType } → returns presigned URL
...
export async function POST(req: NextRequest) {
  try {
    ...
    const clientId = (formData.get("clientId") as string) || process.env.BRAND_SLUG || "default";
    ...
    // Use knr-paris as the brand prefix, with clientId subfolder if provided
    const prefix = clientId ? `brands/knr-paris/${clientId}` : "brands/knr-paris";
    const key = `${prefix}/${assetType}/${uuid()}.${ext}`;
